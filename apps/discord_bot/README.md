...
本体の `src/opsyne/`、依存設定、DB、起動処理への変更はありません。
...
**本体Controlとの接続は未統合です。既定では承認ボタンに「Control APIは未接続」と返します。**
本番操作を行う機能はBotにありません。本体側の本人認証・固定計画照合・承認・認可が必要です。
Discordの実アカウントでの送受信、公開HTTPS配備は未検証です。
...
- [Control bridge接続提案](docs/control-bridge.md): 署名原文、本人照合、固定計画、確認ID、冪等性、応答仕様。

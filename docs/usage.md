...
3. `operator` に切り替え、「案件」で **サンプル: 注文処理を復旧してください** を開き、`demo-restore` を選んで理由を入力し、計画を作成します。
4. `owner` に切り替えて計画を開き、根拠、影響、固定された操作、成功・中止条件、対象版、digestを確認します。確認欄にチェックを入れ、「承認して復旧を実行」を押します。
5. 画面を開いたまま待つと、承認→復旧処理→復旧確認の進捗を表示します。操作が `SUCCEEDED`、確認が `PASS` になった業務障害案件を解決済みにします。「案件と実行記録を確認」で保存結果を確認できます。

承認専用の `reviewer` には「この計画を承認」を表示します。その場合は、承認後に `owner` または `operator` が「復旧処理を開始」から内容を確認し、「復旧を実行して確認」を押します。提案者本人による承認はできません。

この画面は既存の承認・実行・確認APIを同じ認証主体で順番に呼び出します。API単体の承認は実行を開始しません。画面を閉じる、再読み込みする、接続主体を切り替えると、後続処理の開始は保証できません。開き直して案件と実行記録の最新状態を確認してください。

操作結果が不明、通信エラー、確認失敗の場合は画面に残して表示し、自動で操作を再送しません。操作成功後の確認だけ失敗した場合は、実行記録の「独立確認を実行」で再確認できます。AIの調査文からの固定計画の自動生成と、ブラウザ終了後も継続するサーバー側の実行は [Issue #12](https://github.com/siro103/OpSyne/issues/12) の対象です。
...
4. 「変換定義」画面で内容・意味と標本を確認し、別主体が承認します。承認後の新しいログから使用し、過去ログには再解析を実行します。
...
HTTP 2xxでも業務復旧は未確認です。「承認して復旧を実行」「復旧を実行して確認」では操作成功後に独立確認まで進みます。API単体での実行や画面を閉じて確認が中断した場合は、実行記録から独立確認を行ってください。通信途絶・408・5xx等で操作結果が `UNKNOWN` になると保留を維持します。汎用HTTPの「照合」は正本の操作履歴を取得できないため、`UNKNOWN` を解消せず、再送しません。接続先固有の履歴照合が必要です。
